...
class robot:
    ...
    def move(self, motion):
        ...
        new_robot = robot(l)
        new_robot.set_noise(bearing_noise, steering_noise, distance_noise)

        # applying noise
        steering_angle += random.gauss(steering_angle, steering_noise)
        forward += random.gauss(forward, distance_noise)
    ...
    def sense(self, add_noise=0.0):
        distances = []
        [x, y, theta] = [self.x, self.y, self.orientation]
        for i in range(len(landmarks)):
            distances.append((atan2(landmarks[i][0] - y, landmarks[i][1] - x) - theta +
                              add_noise * random.gauss(0.0, bearing_noise)) % (2 * pi))
...
#  TEST CASES
#    Calling the particle_filter function with the following
