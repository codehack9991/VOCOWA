...
# smooth coordinates
# If your code is timing out, make the tolerance parameter larger to decrease run time.
# -----------------------------------------------------------------------
def smooth(tpath, weight_data=0.1, weight_smooth=0.1, tolerance=0.00001):
    ...
    while abs(delta) >= tolerance:
        for i in range(len(tpath)):
            for j in range(len(tpath[0])):
                delta = weight_data * (tpath[i][j] - newpath[i][j]) + weight_smooth * (
                    newpath[i - 1][j] + newpath[(i + 1) % len(newpath)][j] - 2.0 * newpath[i][j])
